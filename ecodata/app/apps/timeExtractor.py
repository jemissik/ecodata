...
file_input = pn.widgets.FileInput(accept="*.", multiple=True)
file_format_input = pn.widgets.TextInput(placeholder="Enter file format")
button = pn.widgets.Button(name="Print File Format")
fileNames = []

file_names_output = pn.pane.Str()
def print_file_format(event):
    user_input = file_format_input.value
    selected_files = "\n".join(file_input.filename)
    fileNames = file_input.filename
    # print(f"The file name is {file_input.filename}")
    # print(f"User Input: {user_input}")
    # print(f"Selected Files:\n{selected_files}")
    # print(f"files is:\n{fileNames}")
    file_names_output.object = f"User Input: {user_input}\nSelected Files:\n{selected_files}\nThe array:\n{fileNames}"
    

button.on_click(print_file_format)

dashboard = pn.Column( file_input, file_format_input, button, file_names_output)
